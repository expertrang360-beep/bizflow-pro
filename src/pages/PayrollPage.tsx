...
export default function PayrollPage() {
  ...
  const [runForm, setRunForm] = useState({ pay_period: "", period_start: "", period_end: "" });
  ...
  const handleRunPayroll = async () => {
    ...
    try {
      ...
      const items = activeStaff.map(s => {
        const paye = s.base_salary * 0.07; // Simplified PAYE
        const pension = s.base_salary * 0.08; // 8% employee pension
      ...
      setRunDialog(false);
      setRunForm({ pay_period: "", period_start: "", period_end: "" });
      fetchData();
    } catch (err) {
      toast({ variant: "destructive", title: "Error", description: err instanceof Error ? err.message : "Failed" });
    } finally { setSaving(false); }
              ...
              {staff.map(s => (
                <div key={s.id} className="bg-card rounded-xl border border-border shadow-card p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{s.role || "Staff"} {!s.active && "• Inactive"}</p>
                  </div>
                </div>
              ))}
            </div>
